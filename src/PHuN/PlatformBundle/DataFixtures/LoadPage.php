...
class LoadPage implements FixtureInterface
{
  public function load(ObjectManager $manager)
  {
   ...
   $corpus = $manager->getRepository('PHuNPlatformBundle:Corpus')->find(5);
   //Concatener le nom du projet avec son id pour constituer project_name
   $project_name = $corpus->getName().'_'.$corpus->getId();
    ...
    foreach ($listUrlImg as $urlImg)
    {
      ...
      if(count($splitAlt) == 4){
            ...
            // Si le catalogue n'existe pas, on l'ajoute dans la DB et dans la liste
      ...
      if(count($splitAlt) == 3){
            // On vérifie l'existence du catalogue
            $isCatalogueExist = false;
            foreach ($listCatalogue as $catalogue) {
              if( strcmp($catalogue, $splitAlt[0]) == 0 ){
                $isCatalogueExist = true;
            ...
            // On vérifie l'existence du dossier
            $isSousDossierExist = false;
            $emptySubFolderName = "emptySubFolder" . "_of" . $splitAlt[1];
            foreach ($listSousDossier as $sousDossier) {
